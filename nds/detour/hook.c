...
int unlock_area(const void *p)
{
    ...
    trace_throttled("mprotect()=%d\n", r);

    return r;
}

#if defined(UT)
TEST(detour, unlock_area)
{
    TEST_ASSERT_EQUAL_INT(-1, unlock_area(NULL));
    TEST_ASSERT_EQUAL_INT(0, unlock_area((const void *)0xdead));
}
#endif
...
int set_fast_forward(uint8_t v)
{
    ...
    // 0xe3a03006
    unlock_area((void *)CODE_FAST_FORWARD);
...
#if defined(UT)
TEST(detour, prehook_puts)
{
    TEST_ASSERT_EQUAL_INT(-1, prehook_puts(NULL));
    TEST_ASSERT_EQUAL_INT(0, !!prehook_puts(""));
    TEST_ASSERT_EQUAL_INT(1, !!prehook_puts("test"));
}
#endif
...
static int prehook_printf_chk(int flag, const char *fmt, ...)
{
    ...
    trace_throttled("call %s(flag=%d, fmt=%p)\n", __func__, flag, fmt);
...
#if defined(UT)
TEST(detour, prehook_printf_chk)
{
    TEST_ASSERT_EQUAL_INT(0, prehook_printf_chk(0, "%d", 100));
    TEST_ASSERT_EQUAL_INT(-1, prehook_printf_chk(0, NULL));
}
#endif

static int32_t prehook_load_state_index(
    void *nds_system,
    uint32_t index,
    uint16_t *d0,
    uint16_t *d1,
    uint32_t shot_only)
{
    ...
    trace_throttled("call %s(index=%d, shot_only=%d)\n", __func__, index, shot_only);
    ...
    snprintf(
        buf,
        sizeof(buf),
        "/roms/nds/savestates/%s_%d.dss",
        (const char *)myhook.var.system.gamecard_name,
        index
    );

    trace_throttled("loading state from: \"%s\"\n", buf);
    r = pfn((void *)myhook.var.system.base, buf, d0, d1, shot_only);

    if (shot_only == 0) {
        if (myhook.reset_menu) myhook.reset_menu();
    }

    // Clear fast forward on load
    uint32_t *ff_bit = (uint32_t*)myhook.var.fast_forward;
    if (ff_bit) {
        *ff_bit &= ~NDS_KEY_BIT_FAST;
    }
    ...
    uint16_t *d1)
{
    ...
    trace_throttled("call %s(index=%d)\n", __func__, index);
    ...
    trace_throttled("saving state to: \"%s/%s\"\n", dir, buf);
    r = pfn((void *)myhook.var.system.base, dir, buf, d0, d1);

    if (myhook.reset_menu) myhook.reset_menu();

    /* OSD Notification for manual save
    if (myhook.manual_save_triggered) {
        nds_print_string pfn_print = (nds_print_string)myhook.fun.print_string;
        if (pfn_print) {
            pfn_print(" MANUAL SAVE WRITTEN ", 0x00ff00, 0x000000, 10, 10);
        }
    } */
...
static void prehook_initialize_backup(
    ...
    char *path)
{
    ...
    trace_throttled("gamecard_name=\"%s\"\n", (const char *)myhook.var.system.gamecard_name);
    ...
    if (path && path[0]) {
        ...
        snprintf(
            data_file_name,
            filename_size,
            "/roms/nds/backup/%s.dsv",
            (const char *)myhook.var.system.gamecard_name
        );

        trace_throttled("new backup path=\"%s\" (orig path=\"%s\")\n", data_file_name, path);
    }
    ...
    do {
        ...
        trace_throttled("init backup struct: type=%d, size=%d\n", backup_type, size);
        ...
        if (backup_type != BACKUP_TYPE_FLASH) {
            if (backup_type == BACKUP_TYPE_NONE) {
                ...
                trace_throttled("backup type NONE, exiting early\n");
                break;
            }

            if (backup_type != BACKUP_TYPE_EEPROM) {
                trace_throttled("backup type not EEPROM, continuing\n");
                break;
            }
            ...
            if (size < 0x10001) {
                ...
                trace_throttled("EEPROM address bytes set to %d\n", uVar5);
                break;
            }
        }
        backup->address_bytes = 3;
        trace_throttled("FLASH/Large EEPROM address bytes set to 3\n");
    } while(0);
    ...
    do {
        ...
        if (data_file_name == (char *)0) {
            trace_throttled("no backup filename, clearing path\n");
            ...
            if (__stream == (FILE *)0) {
                ...
                fclose(__stream);
                trace_throttled("loading backup file %s, %d bytes\n", data_file_name, uVar2);
                ...
                if (uVar2 < size) {
                    ...
                    if (pvVar4 != (void *)0x0) {
                        ...
                        trace_throttled("found DeSmuME footer at %d. Truncating\n", uVar2);
                    }
                    uVar3 = uVar2 >> 0xe;
                    error(
                        "backup file less than full size (should be %d, loaded %d)\n",
                        size,
                        uVar2
                    );
            ...
            trace_throttled("backup file_path set to: %s\n", backup->file_path);
        }
        backup->status = 0;
    } while(0);
...
#if defined(UT)
TEST(detour, prehook_initialize_backup)
{
    prehook_initialize_backup(NULL, 0, NULL, 0, NULL);
    TEST_PASS();
}
#endif
...
int save_state(int slot)
{
    ...
    myhook.manual_save_triggered = 1;
    d0 = malloc(0x18000);
    d1 = malloc(0x18000);
    ...
    do {
        ...
        if (is_state_hooked == 0) {
            ...
            trace_throttled("manual saving state to: \"%s/%s\"\n", dir, buf);
            pfn((void *)myhook.var.system.base, dir, buf, d0, d1);
        }
    } while(0);
    ...
    myhook.manual_save_triggered = 0;
...
int load_state(int slot)
{
    ...
    if (is_state_hooked == 0) {
        ...
        snprintf(
            buf,
            sizeof(buf),
            "%s/savestates/%s_%d.dss",
            state_path,
            (const char *)myhook.var.system.gamecard_name,
            slot
        );

        trace_throttled("manual loading state from: \"%s\"\n", buf);
...
static int patch_drastic64(uint64_t pos, uint64_t pfn)
{
    ...
    trace_throttled("call %s(pos=0x%lx, pfn=0x%lx)\n", __func__, pos, pfn);

    snprintf(buf, sizeof(buf), "/tmp/%s", DRASTIC64);
    trace_throttled("patch the target file (\"%s\")\n", buf);
    ...
    if (memcmp(src, dst, LEN)) {
        ...
        trace_throttled("patched drastic64 at 0x%lx successfully\n", pos);
    }
    else {
        r = 0;
    }
...
int add_prehook(void *org, void *cb, uint8_t *restore)
{
    ...
    trace_throttled("call %s(org=%p, cb=%p)\n", __func__, org, cb);
    ...
    if (unlock_area(org) >= 0) {
        ...
        if (restore) {
            trace_throttled("backup prehook data\n");
...
int restore_prehook(void *pfn, uint8_t *org)
{
    ...
    trace_throttled("call %s(pfn=%p, org=%p)\n", __func__, pfn, org);
...
int init_hook(const char *home, size_t page, const char *path)
{
    ...
    trace_throttled("call %s(home=%p, page=%ld, path=\"%s\")\n", __func__, home, page, path);
    ...
    if (path && path[0]) {
        ...
        trace_throttled("new state path=\"%s\"\n", path);
...
TEST(detour, init_hook)
{
    ...
    TEST_ASSERT_EQUAL_INT(-1, init_hook(NULL, 0, NULL));
    TEST_ASSERT_EQUAL_INT(0, is_state_hooked);
    TEST_ASSERT_EQUAL_INT(0, init_hook(path, 4096, path));
    TEST_ASSERT_EQUAL_INT(1, is_state_hooked);
    TEST_ASSERT_EQUAL_INT(0, strcmp(path, state_path));
}
#endif
