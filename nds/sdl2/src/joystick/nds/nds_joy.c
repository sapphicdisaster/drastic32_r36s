...
static int read_uart(int fd, char *buf, int len)
{
    ...
    trace_throttled("call %s(fd=%d, buf=%p, len=%d)\n", __func__, fd, buf, len);
...
static int read_joy_cfg(const char *path, cali_t *x, cali_t *y)
{
    ...
    while (fgets(buf, sizeof(buf), f)) {
        if (strcasestr(buf, "x_min=")) {
            x->min = atoi(&buf[6]);
            trace_throttled("joy x_min=%d\n", x->min);
        }
        else if (strcasestr(buf, "x_max=")) {
            x->max = atoi(&buf[6]);
            trace_throttled("joy x_max=%d\n", x->max);
        }
        else if (strcasestr(buf, "x_zero=")) {
            x->zero = atoi(&buf[7]);
            trace_throttled("joy x_zero=%d\n", x->zero);
        }
        else if (strcasestr(buf, "y_min=")) {
            y->min = atoi(&buf[6]);
            trace_throttled("joy y_min=%d\n", y->min);
        }
        else if (strcasestr(buf, "y_max=")) {
            y->max = atoi(&buf[6]);
            trace_throttled("joy y_max=%d\n", y->max);
        }
        else if (strcasestr(buf, "y_zero=")) {
            y->zero = atoi(&buf[7]);
            trace_throttled("joy y_zero=%d\n", y->zero);
        }
        else {
...
int JoystickOpen(SDL_Joystick *j, int idx)
{
    ...
    if (!j) {
        trace_throttled("joystick is null\n");
        return -1;
    }
