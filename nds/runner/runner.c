...
static const char *frag_shader_code =
    ...
    "    tc += HALF.xy;                                                 \n"
    "    vec3 tex = texture2D(frag_tex_main, frag_tex_coord).rgb;       \n"
    "    // if (frag_id < 1.5) {                                        \n"
    "        tex = tex.bgr;                                             \n"
    "    // }                                                           \n"
...
static int init_gles(void)
{
    ...
    myrunner.gles.frag.id = glGetUniformLocation(myrunner.gles.object, "frag_id");
...
static void* runner_handler(void *param)
{
    ...
    while (running) {
        ...
        case SHM_CMD_FLUSH:
            trace_throttled("recv SHM_CMD_FLUSH, tex=%d, pitch=%d, alpha=%d, srt(%d,%d,%d,%d), drt(%d,%d,%d,%d)\n",
            ...
            if (myrunner.shm.buf->tex == TEXTURE_BG) {
                ...
                trace_throttled("background texture in runner, w=%d, h=%d, pitch=%d\n", myrunner.gles.bg.w, myrunner.gles.bg.h, myrunner.shm.buf->pitch);
            ...
            glActiveTexture(GL_TEXTURE0);
            glPixelStorei(GL_UNPACK_ALIGNMENT, 1);
            if (myrunner.shm.buf->tex < TEXTURE_MAX) {
                glBindTexture(GL_TEXTURE_2D, myrunner.gles.texture[myrunner.shm.buf->tex]);
                glUniform1f(myrunner.gles.frag.id, (float)myrunner.shm.buf->tex);
                trace_throttled("rendering tex=%d, swizzle=%s\n", myrunner.shm.buf->tex, (myrunner.shm.buf->tex < 2) ? "YES" : "NO");
        ...
        case SHM_CMD_FLIP:
            frame_cnt += 1;
            trace_throttled("recv SHM_CMD_FLIP\n");
        ...
        case SHM_CMD_QUIT:
            running = 0;
            trace_throttled("recv SHM_CMD_QUIT\n");
            break;
        }
        myrunner.shm.buf->valid = 0;
    }

    trace_throttled("free resources\n");
...
#if defined(GKD_PIXEL2) || (defined(GKD_MINIPLUS) || defined(R36S))
