...
int read_file(const char *path, void *buf, int len)
{
    ...
    trace_throttled("call %s(path=%p, buf=%p, len=%d)\n", __func__, path, buf, len);
...
int reset_config(void)
{
    ...
    myconfig.joy.mode = MYJOY_MODE_KEY;
    myconfig.rjoy.mode = MYJOY_MODE_TOUCH;
...
#if defined(MOTO_XT894) || defined(MOTO_XT897) || defined(FXTEC_QX1000) || defined(R36S)
...
int load_config(const char *home_path)
{
    ...
    strcat(buf, "/");
    strcat(buf, CFG_FILE);
    trace_throttled("config=\"%s\"\n", buf);

#if CFG_USING_JSON_FORMAT
    struct json_object *root = json_object_from_file(buf);

    if (!root) {
        error("failed to read configure file \"%s\"\n", buf);

        err = 1;
        reset_config();
        // Force state_path if missing
        if (!myconfig.state_path[0]) {
            strncpy(myconfig.state_path, "/opt/drastic", sizeof(myconfig.state_path));
        }
    }
    else {
        ...
        JSON_GET_INT(JSON_RJOY_MAX_X, myconfig.rjoy.max_x);
        JSON_GET_INT(JSON_RJOY_ZERO_X, myconfig.rjoy.zero_x);
        JSON_GET_INT(JSON_RJOY_MIN_X, myconfig.rjoy.min_x);
        JSON_GET_INT(JSON_RJOY_MAX_Y, myconfig.rjoy.max_y);
        JSON_GET_INT(JSON_RJOY_ZERO_Y, myconfig.rjoy.zero_y);
        JSON_GET_INT(JSON_RJOY_MIN_Y, myconfig.rjoy.min_y);
        JSON_GET_INT(JSON_RJOY_MODE, myconfig.rjoy.mode);
        JSON_GET_INT(JSON_RJOY_DZONE, myconfig.rjoy.dzone);
        JSON_GET_INT(JSON_RJOY_SHOW_CNT, myconfig.rjoy.show_cnt);
        JSON_GET_INT(JSON_RJOY_CUST_KEY0, myconfig.rjoy.cust_key[0]);
        JSON_GET_INT(JSON_RJOY_CUST_KEY1, myconfig.rjoy.cust_key[1]);
        JSON_GET_INT(JSON_RJOY_CUST_KEY2, myconfig.rjoy.cust_key[2]);
        JSON_GET_INT(JSON_RJOY_CUST_KEY3, myconfig.rjoy.cust_key[3]);
#endif
    ...
    if (myconfig.state_path[0] && stat(myconfig.state_path, &st) == -1) {
        ...
        trace_throttled("created \"%s\" folder\n", myconfig.state_path);
    }
...
int update_config(const char *path)
{
    ...
    if (root) {
        ...
        JSON_SET_INT(JSON_RJOY_MAX_X, myconfig.rjoy.max_x);
        JSON_SET_INT(JSON_RJOY_ZERO_X, myconfig.rjoy.zero_x);
        JSON_SET_INT(JSON_RJOY_MIN_X, myconfig.rjoy.min_x);
        JSON_SET_INT(JSON_RJOY_MAX_Y, myconfig.rjoy.max_y);
        JSON_SET_INT(JSON_RJOY_ZERO_Y, myconfig.rjoy.zero_y);
        JSON_SET_INT(JSON_RJOY_MIN_Y, myconfig.rjoy.min_y);
        JSON_SET_INT(JSON_RJOY_MODE, myconfig.rjoy.mode);
        JSON_SET_INT(JSON_RJOY_DZONE, myconfig.rjoy.dzone);
        JSON_SET_INT(JSON_RJOY_SHOW_CNT, myconfig.rjoy.show_cnt);
        JSON_SET_INT(JSON_RJOY_CUST_KEY0, myconfig.rjoy.cust_key[0]);
        JSON_SET_INT(JSON_RJOY_CUST_KEY1, myconfig.rjoy.cust_key[1]);
        JSON_SET_INT(JSON_RJOY_CUST_KEY2, myconfig.rjoy.cust_key[2]);
        JSON_SET_INT(JSON_RJOY_CUST_KEY3, myconfig.rjoy.cust_key[3]);
#endif

        if (json_object_to_file_ext(buf, root, JSON_C_TO_STRING_PRETTY) == 0) {
            trace_throttled("updated configuration successfully\n");
        }
        else {
            ret = 1;
            trace_throttled("updated configuration successfully\n");
...
int get_path_by_idx(const char *folder, int idx, char *buf, int fullpath)
{
    ...
    trace_throttled("call %s(folder=%p, idx=%d, buf=%p, fullpath=%d)\n", __func__, folder, idx, buf, fullpath);
    ...
    while ((dir = readdir(d)) != NULL) {
        ...
        if (cnt == idx) {
            ...
            trace_throttled("found file \"%s\" at index (%d)\n", buf, idx);
            break;
        }
        cnt += 1;
    }
